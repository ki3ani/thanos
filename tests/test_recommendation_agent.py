...
warnings.filterwarnings("ignore", message=".*uses PyType_Spec with a metaclass that has custom tp_new.*", category=DeprecationWarning)
warnings.filterwarnings("ignore", module="google.*", category=DeprecationWarning)
...
def recommendation_main(mock_env_and_deps):
    ...
    # Import the specific main.py from recommendation-agent
    import importlib.util
    import os
    main_path = os.path.join(os.path.dirname(__file__), '..', 'recommendation-agent', 'main.py')
    spec = importlib.util.spec_from_file_location("rec_main", main_path)
    rec_main = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(rec_main)
...
def sample_grpc_product():
    ...
    class MockProduct:
        def __init__(self):
            ...
            self.picture = "test.jpg"
            
            self.price_usd = Mock()
            self.price_usd.currency_code = "USD"
            self.price_usd.units = 10
            self.price_usd.nanos = 500000000
...
class TestRecommendationEndpoint:
    ...
    def test_recommend_success(self, client, recommendation_main, sample_product, mock_env_and_deps):
        ...
        # Setup mocks
        recommendation_main.VALID_PRODUCT_NAMES.clear()
        recommendation_main.VALID_PRODUCT_NAMES.update(["Product A", "Product B", "Test Product"])
    ...
    def test_recommend_product_not_found(self, client, recommendation_main, mock_env_and_deps):
        ...
        # Setup mocks
        recommendation_main.VALID_PRODUCT_NAMES.clear()
        recommendation_main.VALID_PRODUCT_NAMES.update(["Product A", "Product B"])
...
class TestProductCatalogIntegration:
    ...
    def test_get_product_from_catalog_success(self, recommendation_main, sample_grpc_product, mock_env_and_deps):
        ...
        # Setup mocks
        mock_channel = Mock()
        mock_env_and_deps['insecure_channel'].return_value = mock_channel
        
        mock_stub = Mock()
        mock_env_and_deps['stub_class'].return_value = mock_stub
